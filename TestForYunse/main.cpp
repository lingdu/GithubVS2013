...
int main()
{
	//IplImage *src = cvLoadImage("IMG_5794.JPG", CV_LOAD_IMAGE_GRAYSCALE);
	//IplImage *src = cvLoadImage("orgion.jpg", CV_LOAD_IMAGE_GRAYSCALE);
	...
	// ´´½¨×ÓÍ¼Ïñ
	/*IplImage *sub;
	cvSetImageROI(src,cvRect(0,0,src->width/3,src->height/3));
	sub = cvCreateImage( cvSize(src->width/3,src->height/3), src->depth, src->nChannels );
	cvCopy(src,sub);
	cvResetImageROI(src);*/
	cvNamedWindow("Origin", 0);
	cvShowImage("Origin", src);
	CvScalar mean = cvAvg(src);
	//IplImage * B, *G, *R;
	...
	//cvCvtScale(image_re,R,1,0);
	//cvMerge(B,G,R,NULL,src);
	//cvCvtColor(src,src,CV_HSV2BGR);

	cvCvtScale(image_re, srcshift, 255, 0);
	//
	cvNamedWindow("HPF", 0);
	cvShowImage("HPF", srcshift);
	cvSub(src, srcshift, src);
	/*cvCvtScale(src,src,1,mean.val[0]);
